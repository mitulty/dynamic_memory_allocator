...
void *mm_malloc(size_t size)
{
	...
	else
	{
		...
		else
		{
			...
			flag = 0;
			deleteNode(free_list_root, found_node->size, found_node);
			found_node->size |= 1;
		}
	}
	...
	*/
	...
	if (next_block_header != NULL)
	{
		if ((next_block_header->size % 2) == 0 && ((pr_block->size & ~1) + next_block_header->size + 2 * METADATA_SIZE) > size)
		{
			...
			address = (struct free_block *)((char *)next_block_header + METADATA_SIZE);
			...
			parent = free_list_root;
			flag = 0;
			deleteNode(free_list_root, next_block_header->size, address);
...
struct metadata_block *coalesce(struct metadata_block *header)
{
	...
	if (next_node_header != NULL)
	{
		if ((next_node_header->size & 1) % 2 == 0)
		{
			...
			parent = free_list_root;
			flag = 0;
			deleteNode(free_list_root, next_node_header->size, address);
			//		printf("\nReturned From Deleting in Coalescing Next Node\n");
	...
	if (previous_node_footer != NULL)
	{
		if ((previous_node_footer->size & 1) % 2 == 0)
		{
			...
			parent = free_list_root;
			flag = 0;
			deleteNode(free_list_root, previous_node_header->size, address);
...
void show_list_up(void)
{
	...
	while (pr_block != NULL)
	{
		...
		printf("{{Reached at chunk:%p,pr_block->size:%d, free_list_pointer: %p and index:%d}}\n", pr_block, pr_block->size, address, i);
		i++;
		pr_block = pr_block->next_chunk;
	}
	printf("....................................*******************************............................................\n");
