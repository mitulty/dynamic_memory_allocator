...
void *mm_realloc(void *ptr, size_t size)
{
	...
	*/
	struct metadata_block *pr_block = start_head, *next_block_header=start_head->next_chunk, *next_node_footer = NULL, *footer = NULL;
	void *nptr;

	while (((char *)pr_block + METADATA_SIZE) != ptr)
	{
		pr_block = pr_block->next_chunk;
		next_block_header = pr_block->next_chunk;
		if (!pr_block)
			return NULL;
	}
	if ((pr_block->size & ~1) >= size)
	{
		return ptr;
	}
	if (next_block_header != NULL)
	{
		if ((next_block_header->size % 2) == 0 && ((pr_block->size & ~1) + next_block_header->size + 2 * METADATA_SIZE) > size)
		{
		pr_block->size &= ~1;
		next_node_footer = (struct metadata_block *)((char *)next_block_header + next_block_header->size + METADATA_SIZE);
		footer = (struct metadata_block *)((char *)pr_block + pr_block->size + METADATA_SIZE);
		pr_block->size += (2 * METADATA_SIZE) + next_block_header->size;
		pr_block->size |=1;
		next_node_footer->size = pr_block->size;
		pr_block->next_chunk = next_block_header->next_chunk;
...
struct metadata_block *get_free_block(struct metadata_block **last, size_t size) //Best Fit
{
	...
	int min_size = (20 * (1 << 20)); // For Best Fit
	//	int max_size=0;// For Worst Fit
	//	size_t size_needed = size + 2 * METADATA_SIZE;// For Splitting. Use 'size' for no splitting

	while (pr_block != NULL)
	{
		if (pr_block->size >= size && pr_block->size <= min_size && ((pr_block->size) % 2 == 0)) //Best Fit
		//	   if (pr_block->size >= size && ((pr_block->size )%2 == 0))//First Fit
		//if (pr_block->size >= size && pr_block->size >= max_size && ((pr_block->size )%2 == 0))//Worst Fit
		{
	...
	if (min_block_header != NULL)
	{
		...
		min_block_footer->size |= 1;
		min_block_header->size |= 1;
	}
...
void coalesce(struct metadata_block *header)
{
	...
	{
		if ((previous_node_footer->size) % 2 == 0)
		{
